...
function StatusCard({ status, isLoading = false }) {
  if (!status) {
    return (
      <div className="bg-white p-6 rounded-lg mb-5 shadow-md">
        ...
        <div className="text-center py-5 text-gray-600">
          {isLoading ? (
            <span className="inline-block animate-pulse">Loading status...</span>
          ) : (
  ...
  return (
    <div className="bg-white p-6 rounded-lg mb-5 shadow-md">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-5">
        <div className="p-4 bg-gray-50 rounded-lg border-l-4 border-indigo-500 flex gap-2 items-baseline">
        ...
        <div className="p-4 bg-gray-50 rounded-lg border-l-4 border-indigo-500 flex gap-2 items-baseline">
          <label className="block text-gray-600 text-sm">Current Iteration:</label>
          <div className="text-2xl font-bold text-indigo-500">
            {status.current_iteration !== null ? status.current_iteration : '-'}
          </div>
        </div>
        <div className="p-4 bg-gray-50 rounded-lg border-l-4 border-indigo-500 flex gap-2 items-baseline">
          <label className="block text-gray-600 text-sm">Best Accuracy:</label>
          <div className="text-2xl font-bold text-green-600">
            {status.best_accuracy !== null ? `${status.best_accuracy.toFixed(2)}%` : '-'}
          </div>
        </div>
        <div className="p-4 bg-gray-50 rounded-lg border-l-4 border-indigo-500 flex gap-2 items-baseline">
          <label className="block text-gray-600 text-sm">Total Iterations:</label>
          <div className="text-2xl font-bold text-gray-800">{status.total_iterations || 0}</div>
        </div>
        <div className="p-4 bg-gray-50 rounded-lg border-l-4 border-indigo-500 flex gap-2 items-baseline">
          <label className="block text-gray-600 text-sm">Rollbacks:</label>
